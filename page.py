...
@app.route('/detail_information/<int:e>', methods=['GET', 'POST'])
def detail_info(e):
    db = sqlite3.connect("DB_project_data.db")
    db.row_factory = sqlite3.Row
    item = db.execute(
...
@app.route('/comment/<int:eID>', methods=['GET', 'POST'])
def comment(eID):
    db = sqlite3.connect("DB_project_data.db")
    c=db.cursor()
    ...
    for r in data:
        db.execute("INSERT INTO COMMENT(cID,cName,content,eID) values(?,?,?,?)",(r, USER_NAME,request.form['user_message'],eID))
    db.commit()
    db.close()
    return redirect(url_for('detail_info',e=eID))
